...
def make_object_tracker(objName, library, localSpace) -> str:
    return objName if library == None else ":".join([library, objName, str(localSpace)])

def trace_node_id(bucket, objName, library, localSpace) -> int:
    tracker = make_object_tracker(objName, library, localSpace)
