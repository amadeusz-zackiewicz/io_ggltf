...
def node_to_node_unsafe(bucket: Bucket, childNodeAccessor, parentNodeAccessor):
    childNode = RM.fetch_last_id_from_unsafe(bucket, childNodeAccessor, BUCKET_DATA_NODES)
    parentNode = RM.fetch_last_id_from_unsafe(bucket, parentNodeAccessor, BUCKET_DATA_NODES)
    node_to_node(bucket, childNode, parentNode)

def mesh_to_node_unsafe(bucket: Bucket, meshAccessor, nodeAccessor):
    mesh = RM.fetch_last_id_from_unsafe(bucket, meshAccessor, BUCKET_DATA_MESHES)
    node = RM.fetch_last_id_from_unsafe(bucket, nodeAccessor, BUCKET_DATA_NODES)
    mesh_to_node(bucket, mesh, node)

def skin_to_node_unsafe(bucket: Bucket, skinAccessor, nodeAccessor):
    skin = RM.fetch_last_id_from_unsafe(bucket, skinAccessor, BUCKET_DATA_SKINS)
    node = RM.fetch_last_id_from_unsafe(bucket, nodeAccessor, BUCKET_DATA_NODES)
